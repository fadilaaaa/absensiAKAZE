    ...
    if entryInput.get() != "":
        with open('dataname.txt', 'r') as file:  # baca database nama
            ...
            if entryInput.get() not in data:
                ...
                for val in data:
                    entryName['menu'].add_command(
                        label=val, command=tk._setit(stringVar, val))
...
def see_members():
    ...
    for idx in range(len(data)):
        Label(tableFormSeeMembers, text=idx+1, padx=5,
              pady=2).grid(row=idx+1, column=0, padx=1, pady=1, sticky="nsew")
        Label(tableFormSeeMembers, text=data[idx], padx=5, pady=2).grid(
...
def add_new_attendance():
    ...
    btnSaveNewMember = Button(
        formNewAtt, text="Save", command=lambda: save_new_member(entryInput, window=new_attendance))
...
entryName = OptionMenu(formFrame, stringVar, defaultValue,
                       *listNames, command=callbackOptionMenuName)
...
btnSave = Button(submitFormFrame, text="Snap and save",
                 command=lambda: save_file(entryNameStr))
    ...
    if len(coords) != 0:
        ...
        if imgSnap != None:
            ...
            if score < 6:
                ...
                labelName.configure(text="{} ({:3f})".format(
                    pred, score), font=("Arial", 12, "bold"))
